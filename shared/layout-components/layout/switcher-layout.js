...
const Switcherlayout = ({ children }) => {
  const Add = () => {
    ...
    document.querySelector('body').classList.remove('app', 'sidebar-mini', 'ltr', 'landing-page', 'horizontalmenu');
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('Spruhahorizontal')) {
      ...
      document.querySelectorAll('.menu-icon').forEach((e) => e.classList.add('hor-icon'));
      ...
      document.querySelectorAll('.menu-icon').forEach((e) => e.classList.remove('sidemenu-icon'));
    }
