...
const SideBar = () => {
      ...
      menuitems.filter((mainlevel) => {
        if (mainlevel.Items) {
          mainlevel.Items.filter((items) => {
            ...
            if (location.pathname === '/spruha/preview/' || location.pathname === '/spruha/preview') {
            ...
            if (items.children) {
              items.children.filter((submenu) => {
                ...
                if (
                  location.pathname == '/components/ecommerce/product-detail/[id]' &&
  ...
  function Onhover() {
    if (document.querySelector('.main-body')) {
      if (document.querySelector('.main-body').classList.contains('main-sidebar-hide'))
  ...
  return (
    <Fragment>
      <div className="sticky " style={{ marginBottom: '-64px' }} onScroll={horizontalmenusticky()}>
            ...
            <div className="main-container-1 active main-sidebar-header">
              <div className="sidemenu-logo">
                <Link className="main-logo" href={`/components/dashboard/dashboard`}>
                  <img src={logolight.src} className="header-brand-img desktop-logo" alt={'logo1'} />

                  <img src={iconlight.src} className="header-brand-img icon-logo" alt={'logo-2'} />

                  <img src={logo.src} className="header-brand-img desktop-logo theme-logo" alt={'logo-3'} />
                  <img src={icon.src} className="header-brand-img icon-logo theme-logo" alt={'logo-4'} />
                </Link>
              </div>
              <div
                  ...
                  {menuitems.map((Item, itemi) => (
                    <Fragment key={itemi + Math.random() * 100}>
                      ...
                      {Item.Items.map((menuItem, i) => (
                        <li className={`nav-item ${menuItem.selected ? 'active' : ''}`} key={i}>
                          {menuItem.type === 'sub' ? (
                            <a
                              href="#!"
                            ...
                            >
                              ...
                              <div className="according-menu">
                                {menuItem.active ? (
                                  <i className={`${menuItem.background} fa angle fa-angle-down `}></i>
                                ) : (
                                    ...
                                    {childrenItem.children ? (
                                      ...
                                      >
                                        {childrenItem.children.map((childrenSubItem, key) => (
                                          <li
                                            className={`nav-sub-item ${childrenSubItem.selected ? ' active' : ''}`}
                                            key={key}
                                          >
                                            {childrenSubItem.type === 'link' ? (
                                              <Link
                                                ...
                                                className={`nav-sub-link ${
                                                  location.pathname == childrenSubItem.path ? ' active' : ''
                                                }`}
                                              >
                                            ...
                                            {childrenSubItem.type === 'sub' ? (
                                              ...
                                              >
                                                <span className="sidemenu-label">{childrenSubItem.title}</span>
